...
听完当天一轮故事（一句一图、点字发音、点句发音），再做 **9 分测试**。第一次按的算分。**9 分满分才能玩一次游戏，并进入下一轮**（听音选词 / 对不对 / 组句进球）。不到 9 分就温柔地再测一遍，没有失败画面。
...
每句课文都有一张**预先画好、写进仓库**的插图（`static/pictures/d01-l01.svg` …），图下带这句中文。上课只读现成文件，不调用生图模型。改课文后运行 `node scripts/bake-pictures.js` 会按句子自动重画并内置。画风仍可在漫画 / 绘本 / 写实之间切换。
